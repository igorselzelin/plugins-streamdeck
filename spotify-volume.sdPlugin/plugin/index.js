...
const { SpotifyVolumeClient } = require('./spotify');
...
const client = new SpotifyVolumeClient(SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_REFRESH_TOKEN, log);
...
plugin.action1 = new Actions({
    ...
    async dialRotate(data) {
        ...
        if (sign === 0) return;
        try { await client.changeVolumeBy(sign * 5); }
        catch (err) { log.error('changeVolumeBy error:', err.message); }
    },

    async dialDown() {
        try { await client.toggleMute(); }
        catch (err) { log.error('toggleMute error:', err.message); }
    }
});
